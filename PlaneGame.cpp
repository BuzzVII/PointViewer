...
int main( int argc, char** argv ){
    ...
    for(int i=0;i<1000;i++){
        ...
        pt.z = ( rand() % 100 ) / 100.f;
        pt.id = i % 100;
        pt.I = int( sqrt( pt.x * pt.x + pt.y * pt.y + pt.z * pt.z ) * 255 );
        pt.G = 0;
        pt.r = ( ( i % 100 * 3 + 255 ) * 4 ) % 255;
        pt.g = ( ( i % 100 * 3  ) ) % 255;
        pt.b = ( ( i % 100 * 5 + 255 ) ) % 255;

            dims[1] = std::min(dims[1],pt.x);
            dims[2] = std::min(dims[2],pt.y);
            dims[3] = std::min(dims[3],pt.z);
        P.push_back(pt);
    }
